...
class LifeAnnualRings {
    ...
    ensureLabelContainer() {
        if (!this.labelContainer) {
            ...
            // 在标签容器上添加触摸事件处理，防止事件传播到 OrbitControls
            this.labelContainer.addEventListener('touchstart', (event) => {
                // 如果触摸的是标签元素，阻止事件传播
                if (event.target.closest('.ring-label')) {
                    event.stopPropagation();
                }
            }, { passive: false, capture: true });
            
            this.labelContainer.addEventListener('touchend', (event) => {
                // 如果触摸的是标签元素，阻止事件传播
                if (event.target.closest('.ring-label')) {
                    event.stopPropagation();
                }
    ...
    displayMilestoneDetail(ringData) {
        ...
        if (!panel || !milestone) {
            if (panel) {
                this.hideMilestoneDetail(panel);
            }
            return;
        }
        ...
        if (panel.classList.contains('visible')) {
            ...
            // 等待淡出动画完成后再更新内容并淡入
            setTimeout(() => {
                this.updateMilestoneContent(panel, ringData, milestone);
                // 触发重排，确保动画生效
                panel.offsetHeight;
    ...
    updateMilestoneContent(panel, ringData, milestone) {
    ...
    createGradientBackground() {
        ...
        gradient.addColorStop(0, '#011245'); // 顶部颜色
        gradient.addColorStop(1, '#47085b'); // 底部颜色
